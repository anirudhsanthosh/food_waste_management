...
async function updatePickupRequest(request, response, next) {
  ...
  try {
    ...
    if (!address && !quantity)
      throw import_Exceptions.Exceptions.invalidRequest("Both address and quantity cant be empty");
    const { user } = newRequest;
    if (user.id !== existingRequest.user_id)
      throw import_Exceptions.Exceptions.invalidRequest("No such request with given id exist in your account");
    const data = {};
